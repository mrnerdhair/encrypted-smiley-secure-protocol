...
const { parseData, CRC16, randHexArray, argsToByte, uint64LE, expmod } = require('./utils')
...
module.exports = class SSP extends EventEmitter {
  ...
  async initEncryption() {
    const res = await Promise.all([
      BigInt(`0x${dh.createDiffieHellman(64).getPrime("hex")}`),
      BigInt(`0x${dh.createDiffieHellman(64).getPrime("hex")}`),
      BigInt(`0x${dh.createDiffieHellman(64).getPrime("hex")}`),
    ])
    ...
    this.keys.hostIntKey = this.keys.generatorKey.mod
    this.keys.hostIntKey = expmod(this.keys.generatorKey, this.keys.hostRandom, this.keys.modulusKey);

    await this.exec('SET_GENERATOR', uint64LE(this.keys.generatorKey))
    await this.exec('SET_MODULUS', uint64LE(this.keys.modulusKey))
    await this.exec('REQUEST_KEY_EXCHANGE', uint64LE(this.keys.hostIntKey))
    this.count = 0
  }
  ...
  createHostEncryptionKeys(data) {
    if (this.keys.key === null) {
      ...
      this.keys.key = expmod(this.keys.slaveIntKey, this.keys.hostRandom, this.keys.modulusKey)
      this.encryptKey = Buffer.concat([uint64LE(Buffer.from(this.keys.fixedKey, 'hex').readBigUint64BE()), uint64LE(this.keys.key)])
